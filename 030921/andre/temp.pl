...
# fetch path and sample id from inputs:
$stem=shift(@ARGV);
$cSample=shift(@ARGV);

$chr=3;
$minBlockSize_master=12;	# what is this?
...
# defines in psl file, which is a blat alignmant file:
$inPsl=$t[0];

# defines iout bed and tab files:
$outBed=$inPsl;
$outBed=~ s/psl$/f1.bed/;
...
$count=0;
foreach $f (glob($_path)) {   # for each of 2 fastq files:
  print "reading $f ...\n";
  $count++;
  #Perl_e_2input_files
  
  open(IN1, "zcat $f | ") || die "file not found";
  # while there are still lines in file, designate each of 4 
  # fastq lines per read to a1-4:
  while($a1=<IN1> and $a2=<IN1> and $a3=<IN1> and $a4=<IN1>) {	
  	# combine lines for each read:
    $id2fq_read{$count}{$id}="$a1$a2$a3$a4";
    # assign $id as read id if a condition is met - not sure what:
    $id=$1 if $a1 =~ /^@([^ ]+) /;chomp($a2);
    #print "$id # $a2 \n";
    # put read sequence in slot with read count and id to identify it:
    $id2read{$count}{$id}=$a2;
  } close(IN1);
}
...
# not sure why the following is here:
%lng;
%all;
